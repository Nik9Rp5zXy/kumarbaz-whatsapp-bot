const { getUser, addUser, updateBalance, setDaily } = require('../database/mongo');
    ...
    switch (command) {
        ...
        case 'gunluk': {
            ...
            if (now - lastDaily < cooldown) {
                ...
                return msg.reply(`⏳ ${await getRandom(troll.cooldown)}\n(${hours}sa ${minutes}dk kaldı)`);
            }
            const reward = 500;
            await updateBalance(userId, reward);
            await setDaily(userId);
        ...
        case 'param': {
            ...
            if (mentioned) {
                const targetId = await resolve(mentioned);
                let targetUser = await getUser(targetId);
                if (!targetUser) targetUser = await addUser(targetId);
